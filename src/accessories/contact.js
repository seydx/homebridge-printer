...
class ContactAccessory {
  ...
  async getService() {
    ...
    if (this.accessory.context.config.marker) {
        ...
        if (response && response['printer-attributes-tag']) {
          ...
          if (markers && markers.length) {
            markers.forEach((marker, index) => {
              if (marker) {
                let filterService = this.accessory.getServiceById(
                ...
                if (!filterService) {
                  ...
                  filterService = this.accessory.addService(
                    this.api.hap.Service.FilterMaintenance,
                    marker,
                    'marker-' + index
                  );

                  filterService.addCharacteristic(this.api.hap.Characteristic.FilterLifeLevel);
                }
              }
            });
          }
          ...
          if (response) Logger.warn(response, this.accessory.displayName);
        }
      } catch (err) {
      ...
      filterServices.forEach((filter) => {
        ...
        this.accessory.removeService(
          this.accessory.getServiceById(this.api.hap.Service.FilterMaintenance, filter.subtype)
        );
      });
    }
    ...
    if (this.accessory.context.config.switchType === 'CHARACTERISTIC') {
      ...
      service.getCharacteristic(this.api.hap.Characteristic.NetworkState).onSet((value) => {
        Logger.info('Switching state not supported yet', this.accessory.displayName);

        setTimeout(() => {
          service.getCharacteristic(this.api.hap.Characteristic.NetworkState).updateValue(!value);
        }, 500);
      });
    } else {
      if (service.testCharacteristic(this.api.hap.Characteristic.NetworkState))
    ...
    service.getCharacteristic(this.api.hap.Characteristic.ResetTotal).on('set', (value, callback) => {
    ...
    service.getCharacteristic(this.api.hap.Characteristic.ContactSensorState).on('change', (value) => {
      if (value.oldValue !== value.newValue) {
        if (value.newValue) {
          ...
          service.getCharacteristic(this.api.hap.Characteristic.LastActivation).updateValue(lastActivation);

          service
            .getCharacteristic(this.api.hap.Characteristic.TimesOpened)
            .updateValue(this.accessory.context.timesOpened);

          service.getCharacteristic(this.api.hap.Characteristic.ClosedDuration).updateValue(closeDuration);
        } else {
          let openDuration = moment().unix() - this.historyService.getInitialTime();

          service.getCharacteristic(this.api.hap.Characteristic.ClosedDuration).updateValue(openDuration);
        }
  ...
  async getState() {
    let switchAccessory = this.accessories.find(
      (accessory) => accessory.displayName === this.accessory.displayName + ' Switch'
    );
  ...
  getPrinterAttributes() {
    return new Promise((resolve, reject) => {
      ...
      this.printer.execute('Get-Printer-Attributes', msg, (err, res) => {
        ...
        if (res.statusCode && res.statusCode !== 'successful-ok') return reject(res);
