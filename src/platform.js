    ...
    this.config.printer.forEach((printer) => {
      ...
      if (!error) {
        ...
        if (this.devices.has(uuidSwitch)) {
          ...
          printer.polling = Number.isInteger(printer.polling) ? (printer.polling < 1 ? 1 : printer.polling) : 10;
...
PrinterPlatform.prototype = {
  ...
  setupAccessory: function (accessory, device) {
    ...
    const manufacturer = device.manufacturer ? device.manufacturer : 'Homebridge';

    const model = device.model ? device.model : device.type;

    const serialNumber = device.serialNumber ? device.serialNumber : 'SerialNumber';
